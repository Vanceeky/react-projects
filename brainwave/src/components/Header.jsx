...
const Header = () => {
    ...
    const [openNaviation, setOpenNaviation] = useState(true);
    const toggleNavigation = () => {
        if (openNaviation) {
            setOpenNaviation(false);
            enablePageScroll();
        }
        else{
            setOpenNaviation(true);
            disablePageScroll();
        }
    };
    ...
    return (
        ...
        <div className={`fixed top-0 left-0 w-full z-50 border-b border-n-6 lg:bg-n-8/90 
            ${openNaviation ? 'bg-n-8' : 'bg-n-8/90 backdrop-blur-sm'
            }`}>
            ...
            <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
                ...
                <nav className={`${openNaviation ? 'flex': 'hidden'} fixed top-[5rem] left-0 right-0 bottom-0 bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent  `}>
                ...
                <Button className="ml-auto lg:hidden" px="px-3" onClick={toggleNavigation}>
                    <MenuSvg openNavigation={openNaviation} />
                </Button>
